...
def main():
 ...
 for name,fn in (("AROME",arome),("HARMONIE",harmonie),("ARPEGE_EU",arpege_europe),("GFS",gfs),("GEFS",gefs)):
  try: series.append(fn())
  except Exception as exc: unavailable.append(name+" ("+str(exc)+")")
 payload={"status":"ok","schema_version":1,"module_version":"1.0.2","area":{"codes":["76","93"],"name":"Occitanie + Provence-Alpes-Côte d’Azur"},"generated_at":datetime.now(timezone.utc).isoformat(timespec="seconds").replace("+00:00","Z"),"series":series,"unavailable":unavailable}
